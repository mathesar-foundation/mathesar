...
from db.records.exceptions import BadSortFormat, SortFieldNotFound, TypeNotOrderable
...
def _is_col_orderable(col):
    ...
    if str(data_type) not in non_orderable_type:
        ...
        error_message = str(data_type) + " is not orderable."
        raise TypeNotOrderable(error_message)

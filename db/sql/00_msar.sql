...
CREATE OR REPLACE FUNCTION
msar.list_records_from_exploration(
  exploration jsonb
) RETURNS jsonb AS $$
DECLARE
  records jsonb;
  base_table_oid bigint;
  columns jsonb;
  sel text;
BEGIN
  base_table_oid := exploration->'base_table_oid';
  columns := exploration->'initial_columns';
  sel := build_selectable_for_explorations(base_table_oid, columns);
END;
$$ LANGUAGE plpgsql;


CREATE OR REPLACE FUNCTION
msar.build_selectable_for_explorations(
  base_table_oid bigint,
  columns jsonb
) RETURNS text AS $$
DECLARE
...
BEGIN
  WITH col_cte AS (
    SELECT (x->>'id')::smallint AS id, x->>'alias' AS alias, x->'join_path' AS jp FROM jsonb_array_elements('[{"id": 2, "alias": "abcd"},{"id": 5, "alias":"efgh", "join_path":[[[2254329, 2], [2254334, 1]],[[2254334, 5], [2254321, 1]]]}]'::jsonb) WITH ORDINALITY AS j(x, idx)
  ), jp_cte AS (
    SELECT
    (col#>>'{0,0}')::oid AS lt_oid,
    (col#>>'{0,1}')::smallint AS lt_attnum,
    (col#>>'{1,0}')::oid AS rt_oid,
    (col#>>'{1,1}')::smallint AS rt_attnum
    FROM (SELECT jsonb_array_elements(jp) FROM col_cte) AS y(col)
  ), sel_base_cte AS (
    SELECT 
    CASE 
      WHEN col_cte.jp IS NULL
      AND has_column_privilege(2254329, col_cte.id, 'SELECT')
    THEN
      format('msar.format_data(%I) AS %I', msar.get_column_name(2254329, col_cte.id), col_cte.alias)
    END as base_sel
    FROM col_cte
  ), join_text_cte AS (
    SELECT 
    CASE 
      WHEN col_cte.jp IS NOT NULL
      AND has_column_privilege((col_cte.jp#>>'{-1,-1,0}')::oid, col_cte.id, 'SELECT')
      AND has_column_privilege(lt_oid, lt_attnum, 'SELECT')
      AND has_column_privilege(rt_oid, rt_attnum, 'SELECT') 
    THEN
      jsonb_build_object(
        'join_sel',
        format(
        ...
        'join_text',
        format(
          'LEFT OUTER JOIN %I.%I AS %I ON %I.%I=%I.%I',
          msar.get_relation_schema_name(rt_oid),
          msar.get_relation_name(rt_oid),
          rt_oid,
          lt_oid,
          msar.get_column_name(lt_oid, lt_attnum),
          rt_oid,
          msar.get_column_name(rt_oid, rt_attnum)
        )
      )
    END AS joins
    FROM jp_cte, col_cte
  ) SELECT distinct *, joins->>'join_sel', joins->>'join_text' FROM join_text_cte, sel_base_cte;
END;
$$ LANGUAGE plpgsql;

...
CREATE OR REPLACE FUNCTION __setup_non_orderable_table() RETURNS SETOF TEXT AS $$
BEGIN
  ...
  INSERT INTO circle_table (circle_col, text_col) VALUES
    ('<(0,0),1>'::circle, 'first'),
    ('<(1,1),2>'::circle, 'second'),
    ('<(2,2),3>'::circle, 'third');
END;
...
CREATE OR REPLACE FUNCTION test_list_records_non_orderable_columns_with_pk() RETURNS SETOF TEXT AS $$
DECLARE
  rel_id oid;
BEGIN
  PERFORM __setup_non_orderable_table();
  rel_id := 'circle_table'::regclass::oid;

  -- Table with circle (non-orderable) column and primary key should still work
  -- because the primary key (id) is orderable
  RETURN NEXT is(
    msar.list_records_from_table(
      tab_id => rel_id,
      limit_ => null,
  ...
  RETURN NEXT is(
    jsonb_array_length(msar.list_records_from_table(
      tab_id => rel_id,
      limit_ => null,
      offset_ => null,
      order_ => null,
...
CREATE OR REPLACE FUNCTION test_list_records_non_orderable_columns_no_pk() RETURNS SETOF TEXT AS $$
DECLARE
  rel_id oid;
  err_msg text;
BEGIN
  -- Create a table with ONLY non-orderable columns and no primary key
  CREATE TABLE only_circle_table (
    circle_col circle,
    text_col text
  );
  INSERT INTO only_circle_table (circle_col, text_col) VALUES
    ('<(0,0),1>'::circle, 'first'),
    ('<(1,1),2>'::circle, 'second');

  rel_id := 'only_circle_table'::regclass::oid;

  -- Attempting to list records from a table with only non-orderable columns
  -- and no primary key should raise a clear error
  RETURN NEXT throws_matching(
    format('SELECT msar.list_records_from_table(%L, null, null, null, null, null)', rel_id),
    'Table has no orderable columns'
  );
END;
$$ LANGUAGE plpgsql;

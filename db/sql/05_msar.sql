...
If no orderable columns exist in the table (all columns are non-orderable types like circle, json,
etc.), this function will raise an exception to avoid the "could not identify an ordering operator"
...
*/
DECLARE
  total_order_array jsonb;
  pkey_order_array jsonb;
  combined_order jsonb;
  result_expr text;
BEGIN
  pkey_order_array := COALESCE(msar.get_pkey_order(tab_id), '[]'::jsonb);
  total_order_array := msar.get_total_order(tab_id);
  
  -- Try primary key first, then fall back to all orderable columns
  combined_order := COALESCE(order_, '[]'::jsonb) || pkey_order_array;
  
  -- If no primary key, try to use all orderable columns
  IF jsonb_array_length(combined_order) = jsonb_array_length(COALESCE(order_, '[]'::jsonb)) THEN
    combined_order := COALESCE(order_, '[]'::jsonb) || total_order_array;
  END IF;
  
  -- Check if we have any orderable columns at all
  IF jsonb_array_length(combined_order) = jsonb_array_length(COALESCE(order_, '[]'::jsonb)) THEN
    RAISE EXCEPTION 'Table has no orderable columns. Cannot determine order for records. '
      'Please add a primary key or ensure the table has at least one column with an orderable type '
      '(e.g., integer, text, date, etc.).'
    USING ERRCODE = '42P17';
  END IF;
  
  SELECT string_agg(format('%I %s', attnum, msar.sanitize_direction(direction)), ', ')
  FROM jsonb_to_recordset(combined_order)
...
DECLARE
  ...
  schema_oid oid;
BEGIN
  -- Check schema privilege first (USAGE)
  SELECT relnamespace INTO schema_oid FROM pg_catalog.pg_class WHERE oid = tab_id;
  IF NOT pg_catalog.has_schema_privilege(schema_oid, 'USAGE') THEN
    RAISE insufficient_privilege USING MESSAGE = 'permission denied for schema ' || schema_oid::regnamespace::text;
  END IF;

...
def test_get_group_augmented_records_relation_metadata_fields(roster_table_obj, group_mode):
    roster, engine = roster_table_obj
    group_by = group.GroupBy(
    ...
    augmented_relation = group.get_group_augmented_records_relation(roster, group_by)
    with engine.begin() as conn:
        res = conn.execute(select(augmented_relation)).fetchall()
    for row in res:
        assert all(
            [
...
def test_smoke_get_group_augmented_records_relation_prefix(roster_table_obj):
    roster, engine = roster_table_obj
    group_by = group.GroupBy(
        ['Student Number'],
        mode=group.GroupMode.PREFIX.value,
...
def test_smoke_get_group_augmented_records_relation_email_preproc(roster_table_obj):
    roster, engine = roster_table_obj
    group_by = group.GroupBy(
        ['Student Email'],
        mode=group.GroupMode.DISTINCT.value,
...
def test_smoke_get_group_augmented_records_relation_uris_preproc(uris_table_obj, preproc):
...
def test_smoke_get_group_augmented_records_relation_extract(times_table_obj):
...
def test_get_group_augmented_records_relation_datetimes_preproc(
...
def test_get_group_augmented_records_relation_datetimes_extract(
...
def test_smoke_get_group_augmented_records_relation_magnitude(magnitude_table_obj, mode):
    ...
    augmented_relation = group.get_group_augmented_records_relation(magnitude, group_by)
    with engine.begin() as conn:
        res = conn.execute(select(augmented_relation)).fetchall()
    for row in res:
        assert all(

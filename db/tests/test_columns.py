...
from db import connection, columns, errors
...
def test_delete_column_success():
    ...
    with patch.object(columns, 'drop_columns_from_table') as mock_drop:
        mock_drop.return_value = 1  # Simulate 1 column dropped
        columns.delete_column(table_oid, column_attnum, connection_name)

    mock_drop.assert_called_once_with(table_oid, [column_attnum], connection_name)


def test_delete_column_non_existent():
    table_oid = 123
    column_attnum = 999  # Non-existent column
    connection_name = 'test_conn'

    with patch.object(columns, 'drop_columns_from_table') as mock_drop:
        mock_drop.return_value = 0  # Simulate 0 columns dropped
...
def test_delete_column_permission_denied():
    table_oid = 123
    column_attnum = 456
    connection_name = 'test_conn'

    with patch.object(columns, 'drop_columns_from_table') as mock_drop:
        mock_drop.side_effect = errors.PermissionError('Permission denied')

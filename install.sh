...
read -r -s -p "Please enter the password to use for the database: " dj_db_password
printf "\n"
...
do
    ...
    then
      ...
      user_database_urls_lst+=("($db_name|postgresql://$db_username:$db_password@mathesar_db:$db_port/$db_name)")
    else
        break
    fi
done
...
tee .env <<EOF
POSTGRES_USER=$dj_db_username
POSTGRES_PASSWORD=$dj_db_password
POSTGRES_HOST=$db_port
ALLOWED_HOSTS=["$allowed_hosts"]
SECRET_KEY=$secret_key
DJANGO_DATABASE_KEY=default
DJANGO_DATABASE_URL=postgres://$dj_db_username:$dj_db_password@mathesar_db:5432/mathesar_django

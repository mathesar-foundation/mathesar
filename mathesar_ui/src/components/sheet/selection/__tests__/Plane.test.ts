...
test('Plane with placeholder row', () => {
  ...
  expect([...p.dataCellsInFlexibleRowRange('r3', 'PL')].length).toBe(8);
  expect([...p.dataCellsInFlexibleRowRange('PL', 'PL')]).toEqual([
    'r4-c1',
    'r4-c2',
    'r4-c3',
    'r4-c4',
  ...
  expect(p.getAdjacentCell('r4-c4', Direction.Right)).toEqual({ type: 'none' });

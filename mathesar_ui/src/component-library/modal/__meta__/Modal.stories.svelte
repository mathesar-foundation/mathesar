...
<Story name="Basic">
  ...
  <Modal isOpen={basicModal} title="Basic modal">
    Here is modal content
    <div slot="footer">
      <p>This is the modal footer</p>
      <p><Button on:click={() => basicModal.close()}>Here is a custom close button</Button></p>
    </div>
  </Modal>

  <Modal isOpen={chainModal} title="Go ahead and open another modal">
  ...
  <Modal isOpen={hardToCloseModal} title="No way to close this one" closeOn={[]}>
    <p>Answer the quesion to unlock the close button.</p>
    <p>2 + 2 = <input type="text" bind:value={answer} /></p>
    <Button
      on:click={() => hardToCloseModal.close()}
      disabled={!answerIsCorrect}
    >Close</Button>
  </Modal>

  <Modal isOpen={titleFreeModal}>
    Wow, this one is small
  </Modal>

  <Modal isOpen={verboseModal}>

<script lang="ts">
  ...
  async function proceed() {
    try {
      ...
      const response = await createTableFromDataFile({
        schema,
      ...
      const previewPage = getImportPreviewPageUrl(
        ...
        response.table.oid,
        {
          ...
          renamedIdColumn: response.renamedColumns.id,
        },
      );
...
<style>
  .import-file-view {
    ...
    background-color: var(--surface-raised-1-background);
    margin-bottom: 2rem;
    --fieldset-boxed-background-color: var(--surface-raised-3-background);
  }

<script lang="ts">
  ...
  import type { AggregationType, RelatedColumns } from '@mathesar/stores/table-data';
  ...
  $: depthColor = `hsl(var(--color-primary-hue), ${Math.max(10, 40 - depth * 5)}%, ${Math.min(98, 96 + depth * 1)}%)`;
  $: borderColor = depth > 0 ? 'var(--color-border-muted)' : 'var(--color-border)';
...
<div class="table-node" class:nested={depth > 0} style="--depth-bg: {depthColor}; --depth-border: {borderColor}">

def custom_preprocessing_hook(endpoints):
    ...
    for (path, path_regex, method, callback) in endpoints:
        ...
        if path.startswith("/api/db/v0/databases/") or path.startswith("/api/db/v0/data_files/") or path.startswith("/api/db/v0/schemas/") or path.startswith("/api/db/v0/tables/") or path.startswith("/api/db/v0/links/"):

...
First, we need to update the software repository and upgrade all packages using the apt command below. SSH to your server.
...
```sh
sudo apt install locales build-essential acl ntp git python3-pip ipython3 zlib1g-dev libncurses5-dev libgdbm-dev libnss3-dev libssl-dev libreadline-dev libffi-dev libsqlite3-dev wget libbz2-dev -y
```

### Step Two: Install PostGreSQL Database Server
...
```
After importing the GPG key, add repository contents to your Ubuntu system:
```sh
...
The repository added contains many different packages including third party addons. They include:
   ...
   - pgadmin packages
With the repository added we can install the PostgreSQL 13 packages on our Ubuntu Linux server. But first update the package index for the version to be available at the OS level.
```sh
sudo apt update
```
...
Before we start, we first need to secure our database server by setting a strong password to the default `postgres` user.
...
### Step Three: Install the Mathesar application
...
`
You can refer to [Mathesar Configuration](../install/configuration.md) for 

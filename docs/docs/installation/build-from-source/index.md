...
1. Next, we have to create a database for storing Mathesar metadata. You either need to be a `SUPERUSER` or `OWNER` of the database. In this guide, we will be setting the user to be `OWNER` of the database instead of a `SUPERUSER`.

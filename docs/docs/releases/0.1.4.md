...
**NOTE**: The guide in these notes only supports upgrading from 0.1.3 to 0.1.4.

### Starting from a Docker compose or script-guided installation

#### Find needed parts

   1. Find your `.env` and `docker-compose.yml` files. If you followed the defaults on the guided script, these will be in the `/etc/mathesar/` directory.
   1. Find your PostgreSQL data volume (if you are using the Docker managed storage for your data). On Linux, this will be under `/var/lib/docker/volumes/`.
      - If you followed the guided installation script on Linux, this will be at `/var/lib/docker/volumes/mathesar_postgresql_data/`
      - Otherwise, it will be in a directory in the docker-managed space

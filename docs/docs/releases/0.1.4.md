...
#### Find needed parts
 ...
 1. Find your `.env` and `docker-compose.yml` files.
...
1. If you are using a Docker container for your database, find your PostgreSQL data volume.
   Run
   ```
 ...
 For the remainder of this upgrade guide, we'll assume the `docker-compose.yml` and `.env` files are both in the `/etc/mathesar` directory, as they will be if you followed defaults when using the guided script to install Mathesar.
...
1. Make backups of files:
   ```
   cp /path/to/docker-compose.yml /path/to/docker-compose.yml.backup

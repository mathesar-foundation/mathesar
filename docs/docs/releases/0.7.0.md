...
- Configure your file storage backend
...
### Additional improvements to concurrency

Mathesar now uses gevent workers for Gunicorn, resolving several long-standing concurrency issues and enabling non-blocking I/O for most operations.

This change **does not** affect the performance of data explorations, which still rely on `psycopg2` through `SQLAlchemy`. However, all other pages and operations—such as database, schema, and table views—will now run concurrently without blocking one another.

*Related work:*

[#4861](https://github.com/mathesar-foundation/mathesar/pull/4861 "Switch to gevent worker for Gunicorn")

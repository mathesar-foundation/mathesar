...
Mathesar 0.11.0 focuses on authentication, access control, and deployment configuration. This release adds GitHub as a single sign-on provider, adds an option to require users to log in via SSO, adds Azure Blob Storage configuration for imported data files, and introduces an experimental mode for provisioning a separate database for each new user.

This release also improves support for relationships between tables in different schemas, narrows where non-admin users can see user account information, removes URL-based file imports, and includes a fix for Data Explorer exports.
...
### Azure Blob Storage for imported data files

Mathesar can now use Azure Blob Storage for imported data files. Administrators can enable it by setting `DATA_FILES_STORAGE_BACKEND=azure` and providing `DATA_FILES_AZURE_CONTAINER` and `DATA_FILES_AZURE_CONNECTION_STRING`.

*Related work:* [#5275](https://github.com/mathesar-foundation/mathesar/pull/5275 "Azure FileStorage configuration")
...
### User columns now list database collaborators
...
Non-admin users can no longer list all Mathesar users through the `users.list` and `users.get` RPC methods.

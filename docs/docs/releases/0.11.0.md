...
Mathesar 0.11.0 introduces several improvements for administrators including a new setting to require SSO login for users, adding GitHub as an SSO provider, narrower permissions for user account information, and supporting Azure Blob Storage for file storage. It also adds native UI support for interlinked tables in different schemas, plus an experimental option for auto-provisioning a separate role and database for each new user.

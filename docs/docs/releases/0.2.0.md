...
1. Rename the old Docker Compose file.
    ```
    mv xMATHESAR_INSTALLATION_DIRx/docker-compose.yml xMATHESAR_INSTALLATION_DIRx/docker-compose.old.yml
    ```

1. Download the 0.2.0 [docker-compose.yml](https://github.com/mathesar-foundation/mathesar/raw/0.2.0/docker-compose.yml) file and place it within `xMATHESAR_INSTALLATION_DIRx`.
...
1. Pull the new Docker image, and restart Mathesar:
    ...
    ```
    docker compose -f xMATHESAR_INSTALLATION_DIRx/docker-compose.yml up --pull always -d
    ```
...
1. Remove all mathesar containers with
    ```
    docker compose -f xMATHESAR_INSTALLATION_DIRx/docker-compose.yml down --rmi all -v
    ```
...
1. Copy the path of your Mathesar installation directory into the box below. Do not include a trailing slash.

...
The Dockerfile now includes a `minimal` production stage for deployments that do not need the bundled PostgreSQL server. The minimal stage collects static assets at build time, runs as an unprivileged `mathesar` user, and starts Mathesar without the inbuilt database fallback.
...
- Fix public schema permissions when provisioning user-owned databases on Azure-hosted PostgreSQL by granting `CREATE` on `public` from the root role in the target database [#5284](https://github.com/mathesar-foundation/mathesar/pull/5284 "Fix public schema permissions"), [#5296](https://github.com/mathesar-foundation/mathesar/pull/5296 "Merge pull request #5284 from mathesar-foundation/fix_public_schema_permissions"), [#5302](https://github.com/mathesar-foundation/mathesar/pull/5302 "Use root_conn for grant create on public"), [#5305](https://github.com/mathesar-foundation/mathesar/pull/5305 "Use root role connection to user DB to `GRANT CREATE` on `public`")
...
!!! note "Django admin is disabled by default"

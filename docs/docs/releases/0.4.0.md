...
???+ note "Need forms, files, or better tools for joined data in Mathesar? We’d love to talk to you!"
...
Mathesar 0.4.0 makes it easier to manage users, import data, and deploy to the cloud. This release adds Single Sign-On (SSO) support using OIDC, so you can log in with your preferred identity provider. You can now paste data directly into a table to create new rows, and view or edit JSON values right from the interface. There are also new 1-click deployment options for getting Mathesar up and running on Digital Ocean and Railway. The release also includes a set of smaller fixes and enhancements across the app.

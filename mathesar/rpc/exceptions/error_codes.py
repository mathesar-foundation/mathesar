...
def get_error_code(err: Exception) -> int:
    ...
    """
    # Check if the error has a PostgreSQL SQLSTATE code and map it first
    try:
        if hasattr(err, 'diag') and hasattr(err.diag, 'sqlstate') and err.diag.sqlstate:
            sqlstate_map = {
                '42501': -30101,  # InsufficientPrivilege / permission denied
                '42P17': -30147,  # InvalidSavepointSpecification (using for orderable columns error)
            }
            if err.diag.sqlstate in sqlstate_map:
                return sqlstate_map[err.diag.sqlstate]
    except Exception:
        pass

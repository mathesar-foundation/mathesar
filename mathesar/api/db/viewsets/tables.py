...
from db.columns.exceptions import (
    ...
    UniqueValueError,
    ExclusionError,
    ColumnMappingsNotFound,
)
from mathesar.api.db.permissions.table import TableAccessPolicy
from mathesar.api.serializers.dependents import (
...
class TableViewSet(
    ...
    def get_queryset(self):
        ...
        return self.access_policy.scope_viewset_queryset(
            self.request,
            Table.objects.prefetch_related("schema", "schema__database", "columns")
            .prefetch("_sa_table", "columns")
            .order_by("-created_at"),
        )
    ...
    def dependents(self, request, pk=None):
        ...
        serializer = DependentSerializer(
            table.get_dependents(types_exclude), many=True, context={"request": request}
        )
        return Response(serializer.data)
    ...
    def split_table(self, request, pk=None):
        ...
        serializer = SplitTableRequestSerializer(
            data=request.data, context={"request": request, "table": table}
        )
        if serializer.is_valid(True):
            ...
            relationship_fk_column_name = serializer.validated_data[
            ...
            response_serializer = SplitTableResponseSerializer(
                data=split_table_response
            )
    ...
    def move_columns(self, request, pk=None):
        table = self.get_object()
        serializer = MoveTableRequestSerializer(
            data=request.data, context={"request": request, "table": table}
        )
        if serializer.is_valid(True):
    ...
    def previews(self, request, pk=None):
        table = self.get_object()
        serializer = TablePreviewSerializer(
            data=request.data, context={"request": request, "table": table}
        )
        serializer.is_valid(raise_exception=True)
        ...
        try:
            ...
            if (
                type(e.orig) is InvalidTextRepresentation
                or type(e.orig) is CheckViolation
            ):
                raise database_api_exceptions.InvalidTypeCastAPIException(
                    e, status_code=status.HTTP_400_BAD_REQUEST, field="columns"
                )
            else:
            ...
            raise database_api_exceptions.UnsupportedTypeAPIException(
                e, field="columns", status_code=status.HTTP_400_BAD_REQUEST
            )
        table_data.update(
            {
    ...
    def existing_import(self, request, pk=None):
        ...
        serializer = TableImportSerializer(
        ...
        serializer = TableSerializer(existing_table, context={"request": request})

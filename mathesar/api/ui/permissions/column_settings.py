    ...
    def scope_queryset(cls, request, qs):
        if not (request.user.is_superuser or request.user.is_anonymous):
            ...
            permissible_database_role_filter = (
                Q(column__table__schema__database__database_role__role__in=allowed_roles)
                & Q(column__table__schema__database__database_role__user=request.user)
            )
            permissible_schema_roles_filter = (
                Q(column__table__schema__schema_role__role__in=allowed_roles)
                & Q(column__table__schema__schema_role__user=request.user)
            )

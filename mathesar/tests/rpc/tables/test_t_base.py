...
def test_tables_add(rf, monkeypatch, mocked_exec_msar_func):
    ...
    actual_table_info = tables.add(
        table_name='newtable', schema_oid=2200, database_id=11, request=request
    )
    call_args = mocked_exec_msar_func.call_args_list[0][0]
    assert actual_table_info == {
        "oid": 1964474,
        "name": "newtable",
...
def test_tables_preview(rf, monkeypatch, mocked_exec_msar_func):
    ...
    # Backend get_preview now returns strings (after db.tables.get_preview
    # stringifies values), so expected preview records are stringified.
...
def test_list_joinable(rf, monkeypatch, mocked_exec_msar_func):
    ...
    actual_dict = tables.list_joinable(
        table_oid=2254329, database_id=11, max_depth=1, request=request
    )
    call_args = mocked_exec_msar_func.call_args_list[0][0]

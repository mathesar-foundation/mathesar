...
def test_create_new(monkeypatch, rf):
    ...
    def mock_set_up_new_for_user(database, nickname, user, sample_data=[], install_types=True):
        if not (
...
def test_connect_existing(monkeypatch, rf):
    ...
    def mock_set_up_preexisting_database_for_user(
            host, port, database_name, nickname, role_name, password, user, sample_data=[], sslmode="prefer", install_types=True
    ):
        if not (
                host == test_host
                and port == test_port
...
def test_create_new_install_types_false(monkeypatch, rf):
    test_database = "mathesar42"
    test_nickname = "Saint Nick"
    request = rf.post("/api/rpc/v0/", data={})
    request.user = User(username="alice", password="pass1234")
    ...
    def mock_set_up_new_for_user(database, nickname, user, sample_data=[], install_types=True):
        if install_types is not False:
            raise AssertionError("install_types should be False")
        return UserDatabaseRoleMap(
            user=user, database=db_model, configured_role=role_model, server=server_model
        )
    ...
    database_setup.create_new(
        database=test_database, nickname=test_nickname,
        install_types=False, request=request
    )


def test_connect_existing_install_types_false(monkeypatch, rf):
    test_database = "mathesar42"
    test_nickname = "Saint Nick"
    test_host = "example.com"
    test_port = 6543
    ...
    def mock_set_up_preexisting_database_for_user(
            ...
            sample_data=[], sslmode="prefer", install_types=True
    ):
        if install_types is not False:
            raise AssertionError("install_types should be False")
        return UserDatabaseRoleMap(
    ...
    database_setup.connect_existing(
        host=test_host, port=test_port, database=test_database, nickname=test_nickname,
        role=test_role, password=test_password, install_types=False, request=request
    )

...
def verify_data_file_data(data_file, data_file_dict):
    ...
    if data_file.associated_table:
        assert data_file_dict['associated_table'] == data_file.table.id
    else:
        assert data_file_dict['associated_table'] is None
    if data_file.associated_schema:
        assert data_file_dict['associated_schema'] == data_file.schema.id
    else:
        assert data_file_dict['associated_schema'] is None
    if data_file.user:
...
def test_data_file_create_with_wrong_extension(client):
    with open('mathesar/tests/textfile.txt', 'rb') as csv_file:
        data = {
            'file': csv_file
        }
        response = client.post('/api/v0/data_files/', data=data)

...
def test_column_update_type_with_extraneous_display_options(column_test_table_with_service_layer_options, client):
    cache.clear()
    table, columns = column_test_table_with_service_layer_options
    colum_name = "mycolumn1"
    column = _get_columns_by_name(table, [colum_name])[0]
    column_id = column.id
    display_options_data = {'type': 'INTEGER'}
    response = client.patch(
        f"/api/db/v0/tables/{table.id}/columns/{column_id}/",
        display_options_data,
    )

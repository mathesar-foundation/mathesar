...
def test_database_create(client, test_db_name):
    ...
    response = client.post('/api/db/v0/databases/', data=data)
    assert response.status_code == 201
    created_database = Database.objects.get(name=test_db_name)
    assert created_database is not None
    assert created_database.deleted is False


def test_database_create_duplicate(client, db_dj_model):
    # Test case for attempting to create a database with a duplicate name
    data = {
        'name': db_dj_model.name,
...
def test_database_update(client, db_dj_model):
    # Test case for updating database details
    new_name = 'updated_test_db'
    data = {
        'name': new_name,
    ...
    response = client.put(f'/api/db/v0/databases/{db_dj_model.id}/', data=data)
    assert response.status_code == 200
    updated_database = Database.objects.get(id=db_dj_model.id)
    assert updated_database.name == new_name


def test_database_update_invalid_id(client):
    # Test case for updating database details with an invalid database ID
    data = {'name': 'invalid_name'}
    invalid_id = 9999
    response = client.put(f'/api/db/v0/databases/{invalid_id}/', data=data)
    assert response.status_code == 404  # Not Found


def test_database_delete(client, db_dj_model):
    # Test case for deleting a database
    response = client.delete(f'/api/db/v0/databases/{db_dj_model.id}/')
    assert response.status_code == 204  # No Content
    deleted_database = Database.objects.filter(id=db_dj_model.id).first()
    assert deleted_database is None


def test_database_delete_invalid_id(client):
    # Test case for deleting a database with an invalid ID
    invalid_id = 9999
    response = client.delete(f'/api/db/v0/databases/{invalid_id}/')
    assert response.status_code == 404  # Not Found

...
from db.schemas.operations.alter import patch_schema_via_sql_alchemy
...
def update_sa_schema(schema, validated_data):
    errors = [base_api_exceptions.ErrorBody(
        ...
        for arg in set(validated_data) - {'name', 'description'}]
    if errors:
        raise base_api_exceptions.GenericAPIException(errors, status_code=status.HTTP_400_BAD_REQUEST)
    if errors:
        raise ValidationError(errors)
    patch_schema_via_sql_alchemy(schema.name, schema._sa_engine, validated_data)
